...
/// <summary>
/// Static formatter for multidimensional arrays. The emitted code keeps
...
/// </summary>
public static class MultiDimensionalArrayFormatter
{
    ...
    public static void GenerateDeserializeCode(LuminLocalFieldData fieldData, StringBuilder sb)
    {
        ...
        for (int i = 0; i < rank; i++)
        {
            sb.AppendLine("            if (dimension" + i + " < 0) throw new global::System.FormatException(\"A multidimensional array dimension cannot be negative.\");");
            sb.AppendLine("            expectedLength *= dimension" + i + ";");
            sb.AppendLine("            if (expectedLength > int.MaxValue) throw new global::System.FormatException(\"The multidimensional array dimensions are too large.\");");
        }
        sb.AppendLine("            if (length != (int)expectedLength) throw new global::System.FormatException(\"The multidimensional array element count does not match its dimensions.\");");
    ...
    private static void AppendJsonReadLevel(StringBuilder sb, int rank, int level, string elementType, string indent)
    {
        ...
        else
        {
            ...
            sb.AppendLine(indent + "{");
            sb.AppendLine(indent + "    throw new global::System.FormatException(\"JSON rows must have equal lengths for a rectangular array\");");
            sb.AppendLine(indent + "}");
        }
    }
