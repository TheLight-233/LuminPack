...
public static class LuminPackExtensionGenerator
{
    ...
    public static string CodeGenerator(LuminDataInfo data, MetaInfo metaInfo, Compilation compilation)
    {
        ...
        // Track types for which WithCompress extensions were generated.
        // Used to seed the recursive BFS that generates element-type compress variants.
        HashSet<string> withCompressGenerated = new HashSet<string>(StringComparer.Ordinal);
        ...
        foreach (var v in data.localFields)
        {
                ...
                currentGenerationTypes.Add(v.TypeName))
            {
                ...
                {
                    GenerateWithCompressExtension(sb, v.TypeName, v, compressFormatter, metaInfo);
                    withCompressGenerated.Add(v.TypeName);
                }
            }
        }

        // ── Recursive BFS: generate WriteValue + WithCompress for all element types ──
        // For Guid[][] we generate WriteValue(Guid[]) + WriteValueWithCompress(Guid[]);
        // for Guid[] we stop (its compress method uses DangerousWriteUnmanagedArray directly).
        {
            var pendingElementTypes = new Queue<string>();
            // Seed: extract element type from every type that had WithCompress generated
            foreach (var t in withCompressGenerated)
            {
                var el = GetCollectionElementTypeName(t);
                if (el != null) pendingElementTypes.Enqueue(el);
            }

            var bfsProcessed = new HashSet<string>(StringComparer.Ordinal);
            while (pendingElementTypes.Count > 0)
            {
                var typeName = pendingElementTypes.Dequeue();
                if (!bfsProcessed.Add(typeName)) continue;

                var fakeLocal = new LuminLocalFieldData { TypeName = typeName, Name = "value" };
                var elFormatter = FormatterDiscovery.GetFormatter(typeName);
                var elCompressFormatter = FormatterDiscovery.GetCompressFormatter(typeName);
                    ...
                    currentGenerationTypes.Add(typeName))
                {
                    ...
                    sb.AppendLine("        {");
                    elFormatter.Item1(fakeLocal, sb);
                    sb.AppendLine("        }");
                    sb.AppendLine();
                ...
                if (elCompressFormatter.Item1 != null && withCompressGenerated.Add(typeName))
                {
                    GenerateWithCompressExtension(sb, typeName, fakeLocal, elCompressFormatter, metaInfo);
                }

                // Recurse: enqueue element type of this type
                var nextEl = GetCollectionElementTypeName(typeName);
                if (nextEl != null) pendingElementTypes.Enqueue(nextEl);
            }
        }
    ...
    /// <summary>
    /// Emits a WriteValueWithCompress + ReadValueWithCompress extension method pair for the given type.
    /// </summary>
    private static void GenerateWithCompressExtension(
        StringBuilder sb,
        string typeName,
        LuminLocalFieldData localData,
        (Action<LuminLocalFieldData, StringBuilder> ser, Action<LuminLocalFieldData, StringBuilder> deser) compress,
        MetaInfo metaInfo)
    {
        sb.AppendLine($"        [global::LuminPack.Attribute.Preserve]");
        sb.AppendLine($"        [global::System.Runtime.CompilerServices.MethodImpl(MethodImplOptions.AggressiveInlining)]");
        sb.AppendLine(metaInfo.IsNet8
            ? $"        public static void WriteValueWithCompress(ref this LuminPackWriter writer, scoped in {typeName} value)"
            : $"        public static void WriteValueWithCompress(ref this LuminPackWriter writer, in {typeName} value)");
        sb.AppendLine("        {");
        ...
        sb.AppendLine($"        [global::System.Runtime.CompilerServices.MethodImpl(MethodImplOptions.AggressiveInlining)]");
        sb.AppendLine(metaInfo.IsNet8
            ? $"        public static void ReadValueWithCompress(ref this LuminPackReader reader, scoped ref {typeName} value)"
            : $"        public static void ReadValueWithCompress(ref this LuminPackReader reader, ref {typeName} value)");
        sb.AppendLine("        {");
    ...
    /// <summary>
    /// Extracts the element type name from an array or List type name.
    /// "global::System.Guid[][]" → "global::System.Guid[]"
    /// "global::System.Collections.Generic.List&lt;global::System.Guid[]&gt;" → "global::System.Guid[]"
    ...
    private static string? GetCollectionElementTypeName(string typeName)
    {
        ...
        // List<T>
        var el = FormatterDiscovery.GetFirstGeneric(typeName);
        if (!string.IsNullOrEmpty(el) && 
            (typeName.Contains("List<") || typeName.Contains("List`")))
            return el;
