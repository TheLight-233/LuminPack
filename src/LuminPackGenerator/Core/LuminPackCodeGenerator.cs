...
public static class LuminPackCodeGenerator
{
	...
	{
		string text = (data.isValueType ? string.Empty : "?");
		FindAllUnmanagedType(data.fields);
		StringBuilder stringBuilder = new StringBuilder();
		HashSet<string> hashSet = new HashSet<string>();
		if (data.classNameSpace != "<global namespace>")
		{
			hashSet.Add(data.classNameSpace);
		}
		foreach (LuminDataField field in data.fields)
		{
			if (!string.IsNullOrEmpty(field.NameSpace) && field.NameSpace != "Your Data NameSpace" && !field.NameSpace.Contains("<"))
			{
				hashSet.Add(field.NameSpace);
			}
			...
			{
				if (!string.IsNullOrEmpty(classField.NameSpace) && classField.NameSpace != "Your Data NameSpace")
				{
					hashSet.Add(classField.NameSpace);
				}
			}
		}
		stringBuilder.AppendLine("// <auto-generated/>");
		stringBuilder.AppendLine("#nullable enable");
		stringBuilder.AppendLine("#pragma warning disable CS0108 // hides inherited member");
		stringBuilder.AppendLine("#pragma warning disable CS0162 // Unreachable code");
		stringBuilder.AppendLine("#pragma warning disable CS0164 // This label has not been referenced");
		stringBuilder.AppendLine("#pragma warning disable CS0168 ");
		stringBuilder.AppendLine("#pragma warning disable CS0219 // Variable assigned but never used");
		stringBuilder.AppendLine("#pragma warning disable CS0618 // Type or member is obsolete");
		stringBuilder.AppendLine("#pragma warning disable CS0649 ");
		stringBuilder.AppendLine("#pragma warning disable CS8600 // Converting null literal or possible null value to non - nullable type.");
		stringBuilder.AppendLine("#pragma warning disable CS8601 // Possible null reference assignment");
		stringBuilder.AppendLine("#pragma warning disable CS8602");
		stringBuilder.AppendLine("#pragma warning disable CS8604 // Possible null reference argument for parameter");
		stringBuilder.AppendLine("#pragma warning disable CS8619");
		stringBuilder.AppendLine("#pragma warning disable CS8620");
		stringBuilder.AppendLine("#pragma warning disable CS8631 // The type cannot be used as type parameter in the generic type or method");
		stringBuilder.AppendLine("#pragma warning disable CS8765 // Nullability of type of parameter");
		stringBuilder.AppendLine("#pragma warning disable CS9074 // The 'scoped' modifier of parameter doesn't match overridden or implemented member");
		stringBuilder.AppendLine("#pragma warning disable CA1050 // Declare types in namespaces.");
		...
		if (data.enableBurst)
		{
			stringBuilder.AppendLine("using global::Unity.Burst;");
		}
		foreach (string item in hashSet)
		{
			if (!defaultNamespace.Contains(item))
			{
				stringBuilder.AppendLine("using global::" + item + ";");
			}
		}
		stringBuilder.AppendLine();
		stringBuilder.AppendLine("//" + data.generatorType);
		stringBuilder.AppendLine("namespace LuminPack.Generated");
		stringBuilder.AppendLine("{");
		string text2 = TypeMetaChecker.BuildParserClassName(data);
		...
		string text5 = "LuminPackParser<" + text3 + ">";
		stringBuilder.AppendLine("    [global::LuminPack.Attribute.Preserve]");
		stringBuilder.AppendLine("    [global::LuminPack.Attribute.LuminPackParser]");
		stringBuilder.AppendLine("    public sealed class " + text2 + " : " + text5);
		...
		if (data.isUnion)
		{
			LuminPackUnionCodeGenerator.UnionCodeGenerator(stringBuilder, data, metaInfo);
			return stringBuilder.ToString();
		}
		switch (data.generatorType)
		{
		case GeneratorType.VersionTolerant:
			LuminPackVersionTolerantCodeGenerator.VersionTolerantCodeGenerator(stringBuilder, data, metaInfo);
			return stringBuilder.ToString();
		case GeneratorType.CircleReference:
			LuminPackCircleReferenceCodeGenerator.CircleReferenceCodeGenerator(stringBuilder, data, metaInfo);
			return stringBuilder.ToString();
		case GeneratorType.NonGenerator:
			return string.Empty;
		default:
		{
			stringBuilder.AppendLine("        static " + text4 + "()");
			stringBuilder.AppendLine("        {");
			stringBuilder.AppendLine("            LuminPackParseProvider.RegisterParsers(new " + text2 + "());");
			stringBuilder.AppendLine("            LuminPackParseProvider.RegisterParsers(new ArrayParser<" + text3 + ">());");
			if (data.isValueType)
			{
				stringBuilder.AppendLine("            LuminPackParseProvider.Cache<" + text3 + "?>.Parser = new NullableParser<" + text3 + ">();");
			}
			stringBuilder.AppendLine("        }");
			stringBuilder.AppendLine();
			stringBuilder.AppendLine("        private " + text4 + "(){}");
			stringBuilder.AppendLine();
			stringBuilder.AppendLine("        [global::LuminPack.Attribute.Preserve]");
			stringBuilder.AppendLine("        [System.Runtime.CompilerServices.MethodImpl(System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]");
			stringBuilder.AppendLine(metaInfo.IsNet8 ? ("        public override void Serialize(ref LuminPackWriter writer, scoped ref " + text3 + text + " value)") : ("        public override void Serialize(ref LuminPackWriter writer, ref " + text3 + text + " value)"));
			stringBuilder.AppendLine("        {");
			GenerateSerializeCode(data, stringBuilder);
			stringBuilder.AppendLine("        }");
			stringBuilder.AppendLine();
			stringBuilder.AppendLine("        [global::LuminPack.Attribute.Preserve]");
			stringBuilder.AppendLine("        [System.Runtime.CompilerServices.MethodImpl(System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]");
			stringBuilder.AppendLine(metaInfo.IsNet8 ? ("        public override void Deserialize(ref LuminPackReader reader, scoped ref " + text3 + text + " value)") : ("        public override void Deserialize(ref LuminPackReader reader, ref " + text3 + text + " value)"));
			stringBuilder.AppendLine("        {");
			GenerateDeserializeCode(data, stringBuilder);
			stringBuilder.AppendLine("        }");
			stringBuilder.AppendLine();
			stringBuilder.AppendLine("        [global::LuminPack.Attribute.Preserve]");
			stringBuilder.AppendLine(metaInfo.IsNet8 ? ("        public override void CalculateOffset(ref LuminPackEvaluator evaluator, scoped ref " + text3 + text + " value)") : ("        public override void CalculateOffset(ref LuminPackEvaluator evaluator, ref " + text3 + text + " value)"));
			stringBuilder.AppendLine("        {");
			stringBuilder.AppendLine("            // 计算总字节长度");
			if (!data.isValueType)
			{
			...
			{
				stringBuilder.AppendLine("            ref var local = ref LuminPackMarshal.As<" + text3 + ", " + TypeMetaChecker.BuildLocalClassName(data) + ">(ref value);");
			}
			stringBuilder.AppendLine("            int totalLength = 1;");
			stringBuilder.AppendLine();
			for (int num = 0; num < data.fields.Count; num++)
			{
				string text6 = ((data.fields[num].IsPrivate || data.fields[num].isProperty) ? "local" : "value");
				if (data.fields[num].FieldType == LuminDataType.Reference && data.fields[num].Type != LuminFiledType.Other)
				{
					if (data.fields[num].Type == LuminFiledType.String)
					{
						stringBuilder.AppendLine("            var " + data.fields[num].Name + "Length = 0;");
						stringBuilder.AppendLine("            if (" + text6 + "." + data.fields[num].Identifier + " != null)");
						stringBuilder.AppendLine("            {");
						stringBuilder.AppendLine("                var " + data.fields[num].Name + "TempValue = " + text6 + "." + data.fields[num].Identifier + ";");
						stringBuilder.AppendLine("                totalLength += evaluator.GetStringLength(ref " + data.fields[num].Name + "TempValue);");
						stringBuilder.AppendLine("            }");
					...
					else
					{
						stringBuilder.AppendLine("            if (" + text6 + "." + data.fields[num].Identifier + " != null)");
						stringBuilder.AppendLine("            {");
						GenerateSerializeLengthCode(stringBuilder, data.fields[num], text6 + "." + data.fields[num].Identifier, 4, 0);
						stringBuilder.AppendLine("            }");
						LuminFiledType type = data.fields[num].Type;
				...
				if (IsUnmanagedFiledType(data.fields[num].Type))
				{
					int num2 = FindNextUnmanagedType(data, num);
					if (num2 != num)
					{
						int num3 = 0;
						for (int num4 = num; num4 <= num2; num4++)
						{
							int.TryParse(GetFixedFieldLength(data.fields[num4]), out var result);
							num3 += result;
						}
						stringBuilder.Append($"            totalLength += {num3};");
						stringBuilder.AppendLine();
						num = num2;
						continue;
					}
				}
				GenerateSerializeLengthCode(stringBuilder, data.fields[num], text6 + "." + data.fields[num].Identifier, 3, 0);
			}
			stringBuilder.AppendLine();
			stringBuilder.AppendLine("            evaluator += totalLength;");
			stringBuilder.AppendLine("        }");
			stringBuilder.AppendLine();
			LuminPackJsonCodeGenerator.GenerateStaticUtf8Fields(stringBuilder, data);
			stringBuilder.AppendLine();
			LuminPackJsonCodeGenerator.GenerateJsonSerialize(stringBuilder, data, text3, metaInfo);
			stringBuilder.AppendLine();
			LuminPackJsonCodeGenerator.GenerateJsonDeserialize(stringBuilder, data, text3, metaInfo);
			stringBuilder.AppendLine();
			GenerateLocalClassStructure(stringBuilder, data);
			stringBuilder.AppendLine();
			foreach (LuminDataField item2 in data.fields.Where((LuminDataField x) => x.ClassFields.Count > 0))
			{
				GeneratorUnsafeAccessorMethod(stringBuilder, item2, item2.ClassFields);
			}
			stringBuilder.AppendLine("    }");
			stringBuilder.AppendLine("}");
			return stringBuilder.ToString();
	...
	private static void GenerateSerializeLengthCode(StringBuilder sb, LuminDataField field, string fieldPath, int indent, int depth, string multList = "_")
	{
		...
		case LuminFiledType.List:
		{
			...
			else if (luminFiledType2 - 15 <= LuminFiledType.UInt)
			{
				...
				{
					sb.AppendLine(text + "if (!evaluator.IsReferenceOrContainsReferences<" + field.ClassName + ">())");
					sb.AppendLine(text + "    totalLength += Unsafe.SizeOf<" + field.ClassName + ">() * " + fieldPath + ".Count;");
		...
		case LuminFiledType.Array:
		{
			...
			else if (luminFiledType - 15 <= LuminFiledType.UInt)
			{
				...
				{
					sb.AppendLine(text + "if (!evaluator.IsReferenceOrContainsReferences<" + field.ClassName + ">())");
					sb.AppendLine(text + "    totalLength += Unsafe.SizeOf<" + field.ClassName + ">() * " + fieldPath + ".Length;");
					sb.AppendLine(text + "else");
				}
		...
		case LuminFiledType.Class:
		{
			...
			{
				sb.AppendLine(text + "totalLength += Unsafe.SizeOf<" + field.ClassName + ">();");
				break;
			}
	...
	public static void GenerateLocalClassStructure(StringBuilder sb, LuminDataInfo dataInfo, HashSet<string> set)
	{
		...
		if (set.Add(GetLocalLayoutKey(dataInfo)))
		{
			GenerateSingleLocalClass(sb, dataInfo);
		}
		foreach (LuminDataInfo item in list)
		{
			if (set.Add(GetLocalLayoutKey(item)))
			{
				GenerateParentClass(sb, item, dataInfo.localFields);
			}
		}
	...
	{
		string text = TypeMetaChecker.BuildParserClassName(data);
		string text2 = data.classFullName;
		_ = data.className + "Parser";
		if (data.isGeneric)
		{
	...
	public static void GenerateDeserializeCode(LuminDataInfo data, StringBuilder sb, bool polymorphism = false)
	{
		...
		bool flag = !data.isValueType && data.RentPoolMethod == null && (data.SelectedConstructor == null || data.SelectedConstructor.Parameters.Count == 0) && data.fields.All((LuminDataField field) => !field.IsPrivate && !field.isProperty);
		bool flag2 = flag && data.fields.All((LuminDataField field) => !ContainsNestedParserCall(field));
		sb.AppendLine();
		...
		for (int num = 0; num < data.fields.Count; num++)
		{
			...
			string text4 = (flag ? ("result." + luminDataField.Identifier) : (luminDataField.Name + "Temp"));
			...
			if (IsMergeableField(data.fields[num]))
			{
				...
				if (num2 != num)
				{
					...
					for (int num3 = num; num3 <= num2; num3++)
					{
						...
						string text5 = (flag ? ("result." + luminDataField2.Identifier) : (luminDataField2.Name + "Temp"));
						if (num3 == num)
						{
							sb.Append("out " + text5);
						}
			...
			GenerateDeserializeCode(sb, luminDataField, text4, "span", "offset", 3, 0, isFirst: true, "", isArray: false, isList: false, "_", isPrivateFiled: false, "", collectionHeadAlreadyRead: false, flag);
	...
	private static bool ContainsNestedParserCall(LuminDataField field)
	{
		...
		foreach (LuminDataField classField in field.ClassFields)
		{
			if (classField.Type == LuminFiledType.Other || ContainsNestedParserCall(classField))
			{
				return true;
			}
		}
